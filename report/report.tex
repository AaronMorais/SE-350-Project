...
\label{alg:wall_clock}
  ...
  \begin{algorithmic}[1]
    \Function{wall\_clock\_process}{}
      ...
      \State send a tick message to myself in 1000ms
      \While{true}
        \State message = receive\_message(NULL)
        \If{clock reset message}
          \State is\_running = true
          \State time\_base = 0 - elappsed\_ms()
          \State release(message)
        \ElsIf {clock set message}
          \State new\_time\_offset = parse\_time(message)
          \If{parse error}
            \State is\_running = false
            \State send error message to the CRT process
            \State continue while loop
          \EndIf
          \State is\_running = true
          \State time\_base = new\_time\_offset - elapsed\_ms()
          \State release(message)
        \ElsIf{clock terminate message}
          \State is\_running = not value of is\_running
          \State release(message)
        \Else
          \State timer\_message = request\_memory\_block()
          \State send timer\_message as tick message to myself 	in 1000ms
          \If{if is\_running}
            \State send current time to CRT for printing
          \Else
            \State release(message)
          \EndIf
        \EndIf
